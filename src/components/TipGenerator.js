...
import Amount from "./TipGenerator/Amount";
import Bill from "./TipGenerator/Bill";
import Peoples from "./TipGenerator/Peoples";
import SelectTip from "./TipGenerator/SelectTip";
import { useDataLayerValue } from "./../Context/DataContext";

const TipGenerator = () => {
  const [
    { amountPerPerson, tipPerPerson, tipPercent, bill, numberOfPerson },
    dispatch,
  ] = useDataLayerValue();
  useEffect(() => {
    if (bill !== "" && tipPercent !== "" && numberOfPerson !== "") {
      dispatch({
        type: "setAmountPerPerson",
        payload: (bill / numberOfPerson).toFixed(2),
      });
      dispatch({
        type: "setTipPerPerson",
        payload: ((tipPercent * amountPerPerson) / 100).toFixed(2),
      });
    }
  }, [tipPercent, bill, numberOfPerson, amountPerPerson]);

  return (
    <div className="h-[100vh] w-full bg-[#C5E4E7] flex flex-col justify-around items-center">
      ...
      <div className="bg-white rounded-xl flex p-7 h-[26rem]">
        <div className="flex flex-col justify-between">
          <Bill />
          <SelectTip />
          <Peoples />
        </div>
        <div className="bg-[#01464B] p-9 ml-7 rounded-lg w-96">
          <div className="flex flex-col justify-between h-full">
            <div className="">
              <Amount text="Tip Amount" amount={tipPerPerson} />
              <Amount text="Total" amount={amountPerPerson} />
            </div>
            <button
              onClick={() => dispatch({ type: "resetState" })}
              className="bg-[#32bdad] text-[#01464B] w-full rounded-sm font-semibold py-2 text-md"
            >
              RESET
            </button>
          </div>
